...
public class Solution {
        ...
        while (assignedJobs.size() < J) {
            ...
            for (int j = 0; j < J; j++) {
                ...
                if (!assignedJobs.contains(suitableJob) && suitableMachine.jobCanbeadded(suitableJob)) {
                    suitableMachine.addJob(suitableJob);
    ...
    private void IEACT() {
        ...
        while (assignedJobs.size() < J) {
            ...
            suitableMachine.addJob(suitableJob, newslot, requiredTools);
            assignedJobs.add(suitableJob);
        }
        ...
        for (int m = 0; m < M; m++) {
            if (m != m1 && m != m2) {
                ...
                for (int j = 0; j < machines[m].jobs.size(); j++) {
                    if (m == m1 && j == j1) {
                        newSolution.machines[m].addJob(machines[m2].jobs.get(j2));
                    } else if (m == m2 && j == j2) {
                        newSolution.machines[m].addJob(machines[m1].jobs.get(j1));
                    } else {
            ...
            if (m != into && m != from) {
                ...
                for (int j = 0; j < machines[m].jobs.size(); j++) {
                    if (m == into && j == beforejob) {
                        newSolution.machines[m].addJob(machines[from].jobs.get(job));
                        newSolution.machines[m].addJob(machines[m].jobs.get(j));
        ...
        for (int m = 0; m < M; m++) {
            if (m != into && m != from) {
                ...
                for (int j = 0; j < machines[m].jobs.size(); j++) {
                    if (m == into && j == beforejob) {
                        for (int k = job; k <= tojob; k++) {
                            newSolution.machines[m].addJob(machines[from].jobs.get(k));
                        }
                        newSolution.machines[m].addJob(machines[m].jobs.get(j));
